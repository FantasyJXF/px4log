...
public:
	...
	PX4LogMessageDescription(char *buffer);
...
PX4LogMessageDescription::PX4LogMessageDescription(char *buffer) {
	...
	uint8_t ofs = 0;
	type = _GET_char(buffer, ofs) & 0xFF;
	ofs += 1;
	length = _GET_char(buffer, ofs) & 0xFF;
	ofs += 1;
...
PX4LogMessage PX4LogMessageDescription::parseMessage(/*streambuf*/char* buffer) {
